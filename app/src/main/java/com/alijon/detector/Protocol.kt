...
data class Telemetry(
    ...
    /**
     * Собственные часы прибора, мс (поле N). null — прошивка старая.
     *
     * Нужны графику проводки: время прихода строки на телефон включает в себя
     * интервал соединения Bluetooth, а он плавает на те же 15..30 мс, которые
     * мы и пытаемся измерить. Со своей меткой прибора эта дрожь пропадает.
     */
    val deviceMs: Long? = null,

    /**
     * Битовая маска сработавших компараторов (поле M), бит 0 — первое деление.
     * Нужна не прибору, а нам: по ней видно, даёт ли линейка сплошную шкалу
     * снизу вверх или деления разбросаны.
     */
    val mask: Int? = null,
)
...
/**
 * Настройки прибора, которые правятся из меню «Прибор».
 *
 * ВНИМАНИЕ: эти команды понимает прошивка начиная с той версии, где добавлен
 * разбор строк DIV/PID/TSP. На более старой прошивке прибор их молча
 * проигнорирует — приложение это увидит по отсутствию ответа CFG и честно
 * скажет, что настройки недоступны.
 *
 *  Запрос:  CFG?
 *  Ответ:   CFG DIV=1000 KP=250 KI=30 KD=10 TSP=20
 *  Запись:  DIV <коэф x1000>
 *           PID <Kp x100> <Ki x100> <Kd x100>
 *           TSP <температура, C>
 *
 * Дробные числа передаются целыми с множителем, чтобы разбор в прошивке
 * оставался целочисленным — на контроллере это и быстрее, и надёжнее.
 */
data class DeviceConfig(
    val divider: Int = 1000,   // коэффициент делителя x1000
    val kp: Int = 0,           // x100
    val ki: Int = 0,           // x100
    val kd: Int = 0,           // x100
    val tempSet: Int = 20,     // уставка подогрева, C
)

object Parser {
    private val idRe = Regex("""(\w+)=([\w.\-]+)""")

    /** Разбирает строку настроек «CFG DIV=1000 KP=250 …». */
    fun config(line: String): DeviceConfig? {
        if (!line.startsWith("CFG ")) return null
        val m = idRe.findAll(line).associate { it.groupValues[1] to it.groupValues[2] }
        fun v(k: String, d: Int) = m[k]?.toIntOrNull() ?: d
        return DeviceConfig(
            divider = v("DIV", 1000),
            kp = v("KP", 0), ki = v("KI", 0), kd = v("KD", 0),
            tempSet = v("TSP", 20),
        )
    }
    ...
    fun telemetry(line: String, prev: Telemetry, keys: List<String>): Telemetry {
        ...
        return prev.copy(
            ...
            setpoints = sp,
            // Метку времени НЕ подхватываем из прошлой строки: устаревшая
            // метка хуже отсутствующей — график разложил бы по ней отсчёты
            // как по свежей. Нет поля — значит прошивка старая, и время
            // ставит телефон.
            deviceMs = num("N")?.toLong(),
            mask = num("M")?.toInt() ?: prev.mask,
        )
    }
}
