...
class VcoAudio(private val ctx: Context? = null) {
    ...
    /*
     * Громкости разведены нарочно.
     *
     * Фон и сигнал слушают по-разному: фон должен быть на грани слышимости и
     * не утомлять за несколько часов проводки, а сигнал — выдёргивать
     * внимание. Одна общая ручка заставляла выбирать между «фон гудит в ушах»
     * и «цель едва слышно».
     */
    @Volatile var bgVolume: Float = 0.35f      // 0..1, фон
    @Volatile var sigVolume: Float = 0.85f     // 0..1, цель

    /** Обычный тон или режим для слабослышащих. */
    @Volatile var mode: SoundMode = SoundMode.NORMAL
    ...
    // Режим для слабослышащих: низкий регистр, где разборчивость держится
    // дольше всего при возрастной потере слуха.
    private val hearLowHz = 150f
    private val hearHighHz = 350f
    ...
    private fun start() {
        ...
        /*
         * РАЗМЕР БУФЕРА САМ ПО СЕБЕ ЗАДЕРЖКУ НЕ ОПРЕДЕЛЯЕТ — ОПРЕДЕЛЯЕТ ТО,
         * НАСКОЛЬКО МЫ ЕГО НАПОЛНЯЕМ.
         *
         * Раньше поток писал в дорожку блокирующим write() без пауз. Такой
         * цикл всегда упирается в полный буфер, а значит в нём постоянно
         * лежит буфер целиком — уже посчитанного, но ещё не прозвучавшего
         * сигнала. getMinBufferSize на 48 кГц даёт 40..80 мс, и ровно
         * столько прибор «опаздывал» сверх всего остального.
         *
         * Меньше минимума дорожку создать нельзя, AudioTrack откажется. Но
         * держать её полной мы не обязаны: ниже поток сам следит за длиной
         * очереди и подливает ровно столько, чтобы звук не рвался.
         */
        ...
        /*
         * Дорожку забираем в локальную переменную один раз.
         *
         * Поле track изменяемое, и обращаться к нему из рабочего потока
         * на каждом круге незачем: пока поток жив, дорожка одна и та же —
         * stop() освобождает её только после join(). Заодно снимается
         * вопрос об умном приведении изменяемого поля к ненулевому типу.
         */
        val trk = track
        if (trk == null) { running = false; return }
        ...
        worker = Thread {
            ...
            var amp = 0.10f
            var brown = 0.0f                 // состояние генератора коричневого шума
            val rnd = java.util.Random()

            /*
             * Учёт очереди.
             *
             * written — сколько кадров всего отдано дорожке, played — сколько
             * она уже проиграла. Разность и есть то, что лежит в очереди и
             * прозвучит с задержкой. Держим её около targetFrames (две пачки,
             * обычно 10..20 мс) вместо целого буфера.
             *
             * Дорожка считает кадры 32-битным счётчиком и переполняется
             * примерно через сутки непрерывной работы, поэтому разность
             * берётся по маске в 32 бита — иначе после переполнения очередь
             * покажется гигантской и звук замолчит навсегда.
             */
            val mask = 0xFFFFFFFFL
            var written = 0L
            var target = (frames * 2).toLong()
            val maxTarget = (bufBytes / 2 - frames).toLong().coerceAtLeast(target)
            var lastUnderrun = 0

            // Незаписанный остаток пачки. Пока он есть, новую не считаем:
            // иначе на месте разрыва будет щелчок.
            var pending = 0
            var offset = 0

            while (running) {
                // 1. Не подливаем, пока очередь набрана. Спим ровно до момента,
                //    когда освободится место, но не дольше 20 мс — чтобы
                //    выключение звука не ждало.
                val played = trk.playbackHeadPosition.toLong() and mask
                val queued = ((written and mask) - played) and mask
                if (pending == 0 && queued >= target) {
                    val waitMs = ((queued - target) * 1000L / rate).coerceIn(1L, 20L)
                    try { Thread.sleep(waitMs) } catch (_: InterruptedException) { break }
                    continue
                }

                // 2. Считаем пачку по САМОМУ СВЕЖЕМУ уровню — он мог прийти по
                //    Bluetooth пока мы спали.
                if (pending == 0) {
                    val t = (level / levels.coerceAtLeast(1)).coerceIn(0f, 1f)
                    val hearing = mode == SoundMode.HEARING

                    val wantFreq =
                        if (hearing) hearLowHz + (hearHighHz - hearLowHz) * t
                        else thresholdHz + (topHz - thresholdHz) * t

                    // Целевая громкость тона: в обычном режиме фон тоже звучит
                    // тоном, в режиме для слабослышащих фоном служит шум, а тон
                    // появляется только на цели.
                    val wantAmp = if (hearing) sigVolume * t else bgVolume + (sigVolume - bgVolume) * t
                    ...
                    for (i in buf.indices) {
                        ...
                        var v = sin(phase).toFloat() * amp

                        if (hearing) {
                            /*
                             * Коричневый шум — это проинтегрированный белый: каждый
                             * следующий отсчёт недалеко уходит от предыдущего, и
                             * спектр валится на 6 дБ на октаву. Звучит как ровный
                             * прибой, а не как шипение, и слышен всем ухом сразу.
                             * Делитель 1.02 не даёт постоянной составляющей
                             * накопиться и увести сигнал в отсечку.
                             */
                            val white = rnd.nextFloat() * 2f - 1f
                            brown = (brown + 0.02f * white) / 1.02f
                            v += brown * 3.5f * bgVolume
                        }

                        buf[i] = (v.coerceIn(-1f, 1f) * 26000).toInt().toShort()
                    }
                    pending = buf.size
                    offset = 0
                }
                ...
                /*
                 * 3. Пишем НЕ блокируясь. Блокирующий write() как раз и
                 *    наполнял буфер до краёв — именно от него мы уходим.
                 *
                 * Дорожка может принять пачку не целиком. Тогда остаток
                 * дописываем на следующем круге, а новую пачку не считаем:
                 * на месте разрыва фазы был бы слышен щелчок.
                 *
                 ...
                 */
                val n = try {
                    trk.write(buf, offset, pending, AudioTrack.WRITE_NON_BLOCKING)
                ...
                if (n > 0) {
                    ...
                    break                      // ошибка дорожки — уходим тихо
                } else {
                    try { Thread.sleep(1) } catch (_: InterruptedException) { break }
                }

                /*
                 * 4. Подрыв означает, что мы подливали слишком скупо и дорожке
                 *    нечего было играть. Добавляем пачку к цели — так очередь
                 *    сама находит минимум, при котором именно этот телефон
                 *    ещё не заикается. Обратно не уменьшаем: колебание туда-сюда
                 *    слышно хуже, чем лишние пять миллисекунд.
                 */
                val u = trk.underrunCount
                if (u != lastUnderrun) {
                    // Первые пачки дорожка почти всегда не успевает принять:
                    // она уже играет, а данных ещё нет. Эти подрывы не в счёт,
                    // иначе очередь вырастет на пустом месте.
                    if (written > frames * 8L) target = (target + frames).coerceAtMost(maxTarget)
                    lastUnderrun = u
                }
            }
