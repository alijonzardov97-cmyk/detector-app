...
class DetectorBle(private val ctx: Context) {
    ...
    /** Настройки прибора. null — прибор их не прислал (старая прошивка). */
    val config = MutableStateFlow<DeviceConfig?>(null)

    /*
     * КОРОТКИЙ ПУТЬ ДЛЯ СТУПЕНИ ЦЕЛИ.
     *
     * Телеметрия уходит в StateFlow, а оттуда — в экран, и это правильно:
     * шкале и графику спешить некуда. Но звуку через ту же дорогу ехать
     * нельзя. Значение попадало бы в генератор тона так: колбэк BLE ->
     * StateFlow -> collectAsStateWithLifecycle -> рекомпозиция ->
     * LaunchedEffect -> audio.level. То есть тон ждал бы кадра Compose и
     * отрисовки шкалы с графиком на 160 точек — лишние 20..40 мс, причём
     * тем больше, чем тяжелее экран.
     *
     * Поэтому ступень отдаётся ещё и напрямую, прямо из разбора строки.
     * Вызывается из потока Bluetooth, поэтому подписчик обязан быть к этому
     * готов: у VcoAudio поле level помечено @Volatile, а Vibrator потокобезопасен.
     * Ничего, что трогает интерфейс или файлы, сюда вешать нельзя.
     */
    var onLevel: ((Int) -> Unit)? = null

    /*
     * Отвод для графика проводки.
     *
     * В отличие от onLevel зовётся на КАЖДУЮ строку телеметрии, а не только
     * при смене ступени. Графику нужны и ровные участки: по ним видно, что
     * связь жива и сигнал действительно держится, а не что строки пропали.
     * Тоже из потока Bluetooth — тяжёлого сюда вешать нельзя.
     */
    ...
    private val gattCb = object : BluetoothGattCallback() {
        ...
        override fun onCharacteristicWrite(
            ...
            // Подтверждение засчитываем только той характеристике, которую ждём.
            // Иначе запоздавший колбэк от другой записи завершал чужое ожидание,
            // и передача рассыпалась дальше по цепочке.
            if (c.uuid != awaitedWrite) return
            awaitedWrite = null
    ...
    private fun handleLine(line: String) {
        ...
        val was = telemetry.value.level
        val now = Parser.telemetry(line, telemetry.value, keys)
        telemetry.value = now
        // Звук и вибрация — сразу, не дожидаясь экрана. Только при изменении
        // ступени: строка приходит и по таймеру, дёргать генератор зря незачем.
        if (now.level != was) onLevel?.invoke(now.level)
        // График — на каждую строку, вместе с собственным временем прибора.
        onSample?.invoke(now.level, now.deviceMs)
    }

    // -------------------------------------------------------------- outgoing
    private var writeAck: CompletableDeferred<Boolean>? = null
    private var awaitedWrite: java.util.UUID? = null
    ...
    /** Спросить прибор о его настройках. Ответ придёт строкой CFG. */
    fun requestConfig() { config.value = null; send("CFG?") }

    /** Записать настройки в прибор. Команды идут по одной, с паузой. */
    suspend fun sendConfig(c: DeviceConfig) {
        send("DIV ${c.divider}");                 delay(60)
        send("PID ${c.kp} ${c.ki} ${c.kd}");       delay(60)
        send("TSP ${c.tempSet}");                  delay(60)
        send("CFG?")                               // сразу перечитываем
    }
    ...
    private suspend fun writeChunk(
        ...
        /*
         * Отказ writeCharacteristic — это чаще всего «стек занят предыдущей
         * операцией», а не поломка. Веб-клиент такого не видит: браузер сам
         * ставит записи в очередь. Здесь очередь наша, поэтому просто ждём и
         * повторяем ту же посылку — иначе целый сектор считался сбойным
         * из-за пустяка, и передача уходила на лишний круг.
         */
        var queued = false
        for (attempt in 0 until 8) {
            if (writeRaw(c, bytes, withResponse)) { queued = true; break }
            delay(20)
        }
        if (!queued) { writeAck = null; awaitedWrite = null; return false }

        val ok = withTimeoutOrNull(5000) { ack.await() } ?: false
        if (!ok) { writeAck = null; awaitedWrite = null }
        return ok
    }
    ...
    suspend fun sendFirmware(image: ByteArray, onProgress: (Float) -> Unit): String? {
        ...
        while (off < size) {
            ...
            /*
             * Размер пакета считаем на КАЖДОМ секторе, а не один раз до цикла.
             * После переподключения посреди передачи MTU договаривается заново
             * и может оказаться меньше прежнего — со старым размером пакеты
             * просто не проходили бы, и сектор падал раз за разом.
             */
