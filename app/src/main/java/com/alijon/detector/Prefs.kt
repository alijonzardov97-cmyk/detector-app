...
class Prefs(ctx: Context) {
    ...
    // --- вид проводки ---
    /** true — поле точек, false — привычный график линией. */
    private val plotDotsState = mutableStateOf(p.getBoolean("plotDots", false))
    var plotDots: Boolean
        get() = plotDotsState.value
        set(v) {
            plotDotsState.value = v
            p.edit().putBoolean("plotDots", v).apply()
        }

    /**
     * На сколько ступеней должен просесть сигнал между двумя целями, чтобы
     * приложение сочло их разными.
     *
     * Одна ступень — самое чуткое разделение, но и шум на грани порога начнёт
     * дробиться на «цели». Три — только уверенные провалы. Двойка посередине.
     */
    private val sepDipState = mutableIntStateOf(p.getInt("sepDip", SweepBuffer.DIP_DEFAULT))
    var sepDip: Int
        get() = sepDipState.intValue
        set(v) {
            sepDipState.intValue = v.coerceIn(1, 3)
            p.edit().putInt("sepDip", sepDipState.intValue).apply()
        }

    /**
     * Размах: 0 — быстро, 1 — средне, 2 — медленно.
     *
     * Задаёт длину окна времени, по которому живут и поле целей, и счётчик.
     * По умолчанию быстрый: при взмахе туда-обратно над одним предметом
     * длинное окно засчитывает две цели вместо одной.
     */
    private val sweepSpeedState = mutableIntStateOf(p.getInt("sweepSpeed", 0))
    var sweepSpeed: Int
        get() = sweepSpeedState.intValue
        set(v) {
            sweepSpeedState.intValue = v.coerceIn(0, 2)
            p.edit().putInt("sweepSpeed", sweepSpeedState.intValue).apply()
        }
