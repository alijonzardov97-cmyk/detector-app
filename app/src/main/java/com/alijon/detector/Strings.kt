...
object S {
    ...
    val whileCharging = S7(
        "идёт заряд", "charging", "wird geladen", "en charge", "cargando", "充电中", "جارٍ الشحن",
    )
    ...
    // --- фон, экран, звук ---
    val bgRunning = S7(
        "Прибор на связи, экран можно гасить",
        "Detector connected, the screen may be off",
        "Gerät verbunden, Bildschirm darf aus sein",
        "Détecteur connecté, l'écran peut être éteint",
        "Detector conectado, la pantalla puede apagarse",
        "探测器已连接，可以熄屏",
        "الجهاز متصل، يمكن إطفاء الشاشة",
    )
    val screenOff = S7("Погасить экран", "Screen off", "Bildschirm aus", "Éteindre l'écran", "Apagar pantalla", "熄灭屏幕", "إطفاء الشاشة")
    val tapToWake = S7(
        "Приложение работает. Нажмите, чтобы вернуть экран.",
        "Running. Tap to bring the screen back.",
        "Läuft. Zum Aufwecken tippen.",
        "En marche. Touchez pour rallumer.",
        "En marcha. Toque para volver.",
        "运行中。点击恢复屏幕。",
        "يعمل. اضغط لإعادة الشاشة.",
    )
    val backgroundWork = S7("Работать в фоне", "Run in background", "Im Hintergrund laufen", "Fonctionner en arrière-plan", "Ejecutar en segundo plano", "后台运行", "العمل في الخلفية")

    val soundSection = S7("Звук", "Sound", "Ton", "Son", "Sonido", "声音", "الصوت")
    val soundNormal = S7("Обычный", "Normal", "Normal", "Normal", "Normal", "普通", "عادي")
    val soundHearing = S7(
        "Для слабослышащих", "For hard of hearing", "Für Schwerhörige",
        "Pour malentendants", "Para hipoacúsicos", "适用于听力障碍", "لضعاف السمع",
    )
    val soundHearingHint = S7(
        "Фон — коричневый шум, сигнал — тон от 150 до 350 Гц. Низкий регистр слышен дольше при потере слуха.",
        "Background is brown noise, the signal is a tone from 150 to 350 Hz. Low registers stay audible longest with hearing loss.",
        "Hintergrund ist braunes Rauschen, das Signal ein Ton von 150 bis 350 Hz. Tiefe Lagen bleiben bei Hörverlust am längsten hörbar.",
        "Le fond est un bruit brun, le signal un ton de 150 à 350 Hz. Les graves restent audibles le plus longtemps.",
        "El fondo es ruido marrón y la señal un tono de 150 a 350 Hz. Los graves se oyen durante más tiempo.",
        "背景为棕色噪声，信号为 150 至 350 赫兹的音调。听力下降时低频保留最久。",
        "الخلفية ضجيج بني والإشارة نغمة من ١٥٠ إلى ٣٥٠ هرتز. النغمات المنخفضة تبقى مسموعة أطول.",
    )
    val volBg = S7("Громкость фона", "Background volume", "Lautstärke Hintergrund", "Volume du fond", "Volumen del fondo", "背景音量", "مستوى الخلفية")
    val volSig = S7("Громкость сигнала", "Signal volume", "Lautstärke Signal", "Volume du signal", "Volumen de la señal", "信号音量", "مستوى الإشارة")

    val brightSection = S7("Яркость экрана", "Screen brightness", "Bildschirmhelligkeit", "Luminosité", "Brillo de pantalla", "屏幕亮度", "سطوع الشاشة")
    val brightAuto = S7("Автоматически", "Automatic", "Automatisch", "Automatique", "Automático", "自动", "تلقائي")
    val brightManual = S7("Вручную", "Manual", "Manuell", "Manuel", "Manual", "手动", "يدوي")
    val noLightSensor = S7(
        "В телефоне нет датчика света — доступна только ручная яркость.",
        "This phone has no light sensor — only manual brightness is available.",
        "Dieses Telefon hat keinen Lichtsensor — nur manuelle Helligkeit.",
        "Ce téléphone n'a pas de capteur de lumière — luminosité manuelle uniquement.",
        "Este teléfono no tiene sensor de luz: solo brillo manual.",
        "本机没有光线传感器，仅支持手动亮度。",
        "لا يحتوي هذا الهاتف على مستشعر ضوء — السطوع اليدوي فقط.",
    )

    val flagLevelTitle = S7(
        "Флажок с деления шкалы", "Flag from scale step", "Fahne ab Skalenstufe",
        "Drapeau à partir du niveau", "Bandera desde el nivel", "自第几格插旗", "العلامة ابتداءً من الدرجة",
    )
    val flagLevelHint = S7(
        "С этого деления считается, что найден металл: ставится флажок и включается GPS.",
        "From this step on, metal is considered found: a flag is dropped and GPS turns on.",
        "Ab dieser Stufe gilt Metall als gefunden: Fahne wird gesetzt und GPS eingeschaltet.",
        "À partir de ce niveau, le métal est considéré comme trouvé : drapeau posé et GPS activé.",
        "A partir de este nivel se considera metal encontrado: se coloca bandera y se activa el GPS.",
        "从该刻度起视为发现金属：插旗并开启 GPS。",
        "ابتداءً من هذه الدرجة يُعدّ المعدن مكتشفًا: تُوضع علامة ويُشغَّل GPS.",
    )
    val gpsHint = S7(
        "GPS включается только при находке и гаснет через полторы минуты. Первая точка в новом месте может оказаться без координат — приёмнику нужно время.",
        "GPS only starts on a find and stops a minute and a half later. The first point in a new area may come without coordinates — the receiver needs time.",
        "GPS startet nur bei einem Fund und schaltet nach anderthalb Minuten ab. Der erste Punkt an einem neuen Ort kann ohne Koordinaten bleiben.",
        "Le GPS ne démarre qu'à une trouvaille et s'arrête une minute et demie plus tard. Le premier point d'un nouveau secteur peut être sans coordonnées.",
        "El GPS solo se activa al detectar y se apaga minuto y medio después. El primer punto en una zona nueva puede quedar sin coordenadas.",
        "GPS 仅在发现目标时开启，一分半后关闭。新区域的第一个点可能没有坐标。",
        "يعمل GPS عند الاكتشاف فقط ويتوقف بعد دقيقة ونصف. قد تكون النقطة الأولى في مكان جديد بلا إحداثيات.",
    )

    val shareCoords = S7("Поделиться", "Share", "Teilen", "Partager", "Compartir", "分享", "مشاركة")
    val shareAllPoints = S7("Отправить все точки", "Send all points", "Alle Punkte senden", "Envoyer tous les points", "Enviar todos los puntos", "发送全部点位", "إرسال كل النقاط")
    val shareCaption = S7("Находка металлоискателя", "Metal detector find", "Fund des Detektors", "Trouvaille du détecteur", "Hallazgo del detector", "金属探测发现", "اكتشاف بجهاز الكشف")
    val enterPinToWrite = S7(
        "Запись настроек в прибор защищена паролем.",
        "Writing settings to the device is password protected.",
        "Das Schreiben der Einstellungen ist passwortgeschützt.",
        "L'écriture des réglages est protégée par mot de passe.",
        "La escritura de ajustes está protegida con contraseña.",
        "写入设备设置需要密码。",
        "كتابة الإعدادات في الجهاز محمية بكلمة مرور.",
    )
    ...
    // --- проводка: вид и разделение целей ---
    val viewTrace = S7("График", "Graph", "Kurve", "Courbe", "Curva", "曲线", "منحنى")
    val viewDots = S7("Точки", "Dots", "Punkte", "Points", "Puntos", "点阵", "نقاط")
    val targetsSeen = S7(
        "Целей за проход: %d",
        "Targets in this pass: %d",
        "Ziele in diesem Zug: %d",
        "Cibles sur ce passage : %d",
        "Objetivos en esta pasada: %d",
        "本次挥扫目标数：%d",
        "الأهداف في هذه المسحة: %d",
    )
    val sepSection = S7(
        "Разделение целей",
        "Target separation",
    ...
    val sepHint = S7(
        "На сколько делений должен просесть сигнал между двумя предметами, чтобы приложение сочло их разными. Одно деление — самое чуткое разделение, но шум начнёт дробиться на «цели». Три — только уверенные провалы.",
        "How far the signal must dip between two objects for the app to count them separately. One step is the most sensitive, but noise starts splitting into “targets”. Three counts only clear dips.",
        "Wie tief das Signal zwischen zwei Objekten einbrechen muss, damit die App sie getrennt zählt. Ein Schritt ist am empfindlichsten, aber Rauschen wird zu „Zielen“. Drei zählt nur deutliche Einbrüche.",
        "De combien le signal doit chuter entre deux objets pour que l’application les compte séparément. Un cran est le plus sensible, mais le bruit se transforme en « cibles ». Trois ne retient que les creux nets.",
        "Cuánto debe caer la señal entre dos objetos para que la aplicación los cuente por separado. Un paso es lo más sensible, pero el ruido se convierte en «objetivos». Tres solo cuenta caídas claras.",
        "两个目标之间信号必须下降多少格，应用才把它们算作两个。一格最灵敏，但噪声会被当成目标；三格只认明显的凹陷。",
        "مقدار انخفاض الإشارة بين جسمين حتى يعدّهما التطبيق هدفين. درجة واحدة هي الأكثر حساسية لكن الضوضاء تتحول إلى «أهداف»، وثلاث درجات تكتفي بالانخفاضات الواضحة.",
    )
    val sepDipLbl = S7(
        "Глубина провала, делений",
        "Dip depth, steps",
        "Einbruchtiefe, Stufen",
        "Profondeur du creux, crans",
        "Profundidad del valle, pasos",
        "凹陷深度（格）",
        "عمق الانخفاض، درجات",
    )
}
